...
const TOTAL_WIDTH = colX(5) + AULA_W; 
const TOTAL_HEIGHT = row3Y + AULA_H;

const AULAS: AulaDesc[] = [
  // Fila Superior
  { id: '4', label: '14', estado: 'NO_DISPONIBLE', x: colX(0), y: row1Y, w: AULA_W, h: AULA_H },
  { id: '3', label: '13', estado: 'NO_DISPONIBLE', x: colX(1), y: row1Y, w: AULA_W, h: AULA_H },
  { id: '2', label: '12', estado: 'NO_DISPONIBLE', x: colX(2), y: row1Y, w: AULA_W, h: AULA_H },
  { id: '1', label: '11', estado: 'NO_DISPONIBLE', x: colX(3), y: row1Y, w: AULA_W, h: AULA_H },
  
  // Fila Inferior
  { id: '5', label: '15', estado: 'NO_DISPONIBLE', x: colX(0), y: row3Y, w: AULA_W, h: AULA_H },
  { id: '6', label: '16', estado: 'NO_DISPONIBLE', x: colX(1), y: row3Y, w: AULA_W, h: AULA_H },
  { id: '7', label: '17', estado: 'NO_DISPONIBLE', x: colX(2), y: row3Y, w: AULA_W, h: AULA_H },
  { id: '8', label: 'S.U.M', estado: 'NO_DISPONIBLE', x: colX(3), y: row3Y, w: AULA_W, h: AULA_H },
  { id: '9', label: 'S.A', estado: 'NO_DISPONIBLE', x: colX(4), y: row3Y, w: AULA_W, h: AULA_H },
];
...
export const InteractiveMap: React.FC = () => {
  ...
  const { aulas: aulasRemotas, fetchAulas } = useAulas();
  ...
  const aulasDinamicas = AULAS.map(aulaEstatica => {
    ...
    if (remota) {
      return { ...aulaEstatica, estado: remota.estado, idRemoto: remota.id };
    }
    return aulaEstatica;
  });
      ...
      <div className="svg-map-wrapper">
        ...
        >
          ...
          <g className="svg-map-content-group">
            ...
            {/* Oficinas y Escaleras - Superiores (Indices 4, 5) */}
            <g>
                ...
                <g>
                   {/* Lineas de Escalera */}
                   <line x1={colX(5)+25} y1={row1Y+20} x2={colX(5)+AULA_W-25} y2={row1Y+20} stroke="rgba(255,255,255,0.15)" strokeWidth={2} />
                   <line x1={colX(5)+25} y1={row1Y+35} x2={colX(5)+AULA_W-25} y2={row1Y+35} stroke="rgba(255,255,255,0.15)" strokeWidth={2} />
                   <line x1={colX(5)+25} y1={row1Y+50} x2={colX(5)+AULA_W-25} y2={row1Y+50} stroke="rgba(255,255,255,0.15)" strokeWidth={2} />
                   <line x1={colX(5)+25} y1={row1Y+65} x2={colX(5)+AULA_W-25} y2={row1Y+65} stroke="rgba(255,255,255,0.15)" strokeWidth={2} />
                   
                   {/* Flecha Doble */}
                   <path d={`M ${colX(5)+AULA_W/2} ${row1Y+12} L ${colX(5)+AULA_W/2} ${row1Y+72}`} stroke="rgba(255,255,255,0.3)" strokeWidth={1.5} />
                   <path d={`M ${colX(5)+AULA_W/2 - 4} ${row1Y+18} L ${colX(5)+AULA_W/2} ${row1Y+12} L ${colX(5)+AULA_W/2 + 4} ${row1Y+18}`} fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth={1.5} />
                   <path d={`M ${colX(5)+AULA_W/2 - 4} ${row1Y+66} L ${colX(5)+AULA_W/2} ${row1Y+72} L ${colX(5)+AULA_W/2 + 4} ${row1Y+66}`} fill="none" stroke="rgba(255,255,255,0.3)" strokeWidth={1.5} />

                   <text x={colX(5) + AULA_W/2} y={row1Y + AULA_H - 12} className="svg-static-text" style={{ fontSize: '10px' }} textAnchor="middle" dominantBaseline="middle">
                       Escaleras
                   </text>
                </g>
            </g>

            {/* Oficina - Inferior (Indice 5) */}
            <g>
                <rect x={colX(5)} y={row3Y} width={AULA_W} height={AULA_H} rx={8} className="svg-static-dashed" />
                <text x={colX(5) + AULA_W/2} y={row3Y + AULA_H/2} className="svg-static-text" textAnchor="middle" dominantBaseline="middle">
                    Oficina
                </text>
            </g>
